    ...
    for (j = 0; j < rw.cells.length; j++)
    {
      ...
      for (k = 0; k < conditions.length; k++)
      {
        ...
        if (conditions[k].name.toUpperCase() == colKey.toUpperCase())
        {
          var tbVal = null;
          if (cl.childNodes[0]) {
            tbVal = cl.childNodes[0].nodeValue;
            if (tbVal == null) tbVal = cl.innerText; //in case we have an html tag in the table cell
            if (tbVal == null) tbVal = cl.childNodes[0].innerHTML; //in case we have an html tag in the table cell (netscape)
          } else tbVal = ' ';
